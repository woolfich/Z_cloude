<script lang="ts">
        ...
        function getFilteredSuggestions(input: string, ratesList: typeof $rates): string[] {
                ...
                // Get articles from rates that are not already in plan
                const existingPlanArticles = new Set($planItems.map(p => p.article));
                const matching = ratesList
                        .filter(r => r.article.startsWith(normalized) && !existingPlanArticles.has(r.article))
                        .map(r => r.article);
        ...
        function handleSubmit() {
                ...
                // Check if article exists in rates
                if (!$rates.some(r => r.article === article)) {
                        alert('Артикул не найден в нормах. Сначала добавьте норму времени.');
                        return;
                }
                ...
                const success = addPlanItem(article, target);
                if (!success) {
                        alert('Позиция для этого артикула уже существует');
                        return;
                }
        ...
        function handleLongPress(planItem: PlanItem) {
                editModal = { show: true, planItem };
                editTargetInput = planItem.target.toString();
        }
        ...
        // Long press action
        function longpressAction(node: HTMLElement, planItem: PlanItem) {
                let currentPlanItem = planItem;
                ...
                function startPress(e: TouchEvent | MouseEvent) {
                        ...
                        timeoutId = setTimeout(() => {
                                ...
                                handleLongPress(currentPlanItem);
                        }, 500);
                }
                ...
                return {
                        update(newPlanItem: PlanItem) {
                                currentPlanItem = newPlanItem;
                        },
                        destroy() {
        ...
        <header class="fixed-header">
                <div class="header-content">
                        ...
                        <div class="input-row">
                                ...
                                <input 
                                        type="text"
                                        class="input-field target-input"
                                        placeholder="План (шт)"
                ...
                {#if $calculatedPlanItems.length === 0}
                        <div class="empty-state">
                                ...
                                <div class="empty-state-text">
                                        Нет позиций плана.<br/>
                                        Добавьте первую позицию выше.<br/>
                                        <br/>
                                        <small>Сначала добавьте нормы времени для артикулов.</small>
                                </div>
                        </div>
                {:else}
                        <div class="plan-list">
                                {#each $calculatedPlanItems as planItem (planItem.id)}
                                        <div 
                                                class="plan-item"
                                                class:locked={planItem.locked}
                                                use:longpressAction={planItem}
                                        >
                                                <div class="plan-left">
                                                        <span class="article-badge" class:article-locked={planItem.locked}>{planItem.article}</span>
                                                        {#if planItem.locked}
                                                                <span class="locked-badge">ПЛАН ВЫПОЛНЕН</span>
                                                        {/if}
                                                </div>
        ...
        <div class="modal-content" onclick={(e) => e.stopPropagation()}>
                ...
                <div class="modal-body">
                        <div class="form-group">
                                <label>План (штук)</label>
                                <input 
                                        type="text"
                                        class="input-field"
                                        bind:value={editTargetInput}
                                        inputmode="decimal"
                                        autocomplete="off"
                                />
                        </div>
