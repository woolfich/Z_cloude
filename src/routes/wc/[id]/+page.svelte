<script lang="ts">
        ...
        import { welders, rates, calculatedPlanItems, addWCEntry, updateWCEntry, deleteWCEntry, setManualOvertime, resetOvertimeOverride } from '$lib/stores';
        import { normalizeArticle, formatQty, getTodayISO, getWelderProgressByArticle } from '$lib/utils';
        ...
        let unlockedPlanItems = $derived($calculatedPlanItems.filter((p: { locked: boolean }) => !p.locked));
        
        // Get entries sorted by updatedAt (most recent first)
        let sortedEntries = $derived(welder 
                ? [...welder.entries].sort((a: WCEntry, b: WCEntry) => b.updatedAt - a.updatedAt)
                : []);
        ...
        // Get today's overtime or most recent overtime
        let currentOvertime = $derived(getCurrentOvertime(welder, today));
        let currentOvertimeDate = $derived(getCurrentOvertimeDate(welder, today));
        ...
        function getFilteredSuggestions(input: string, planItems: { article: string }[]): string[] {
                ...
                if (!normalized) {
                        return planItems.map((p) => p.article).slice(0, 5);
                }
                
                const matching = planItems
                        .filter((p) => p.article.startsWith(normalized) && p.article !== normalized)
                        .map((p) => p.article);
                
                return [...new Set(matching)].slice(0, 5);
        }
        
        function getCurrentOvertime(welder: Welder | undefined, date: string): number {
                if (!welder) return 0;
                return welder.overtime[date] || 0;
        }
        
        function getCurrentOvertimeDate(welder: Welder | undefined, date: string): string {
                if (!welder) return date;
                if (welder.overtime[date] && welder.overtime[date] > 0) {
                        return date;
                }
                const overtimeDates = Object.keys(welder.overtime)
                        .filter((d) => welder.overtime[d] > 0)
                        .sort((a, b) => b.localeCompare(a));
                
                return overtimeDates[0] || date;
        }
        ...
        function handleArticleKeyDown(e: KeyboardEvent) {
                if (e.key === 'ArrowDown') {
                        ...
                        if (suggestionsAvailable && highlightedIndex >= 0) {
                                selectArticle(filteredSuggestions[highlightedIndex]);
                        } else if (articleInput.trim()) {
                                selectArticle(normalizeArticle(articleInput));
                        }
        ...
        function selectArticle(article: string) {
                ...
                const planItem = $calculatedPlanItems.find((p: { article: string }) => p.article === normalized);
                if (!planItem) {
                        alert('Артикул не найден в плане');
                        return;
                }
                ...
                selectedArticle = normalized;
                articleInput = normalized;
                showSuggestions = false;
                stage = 'quantity';
                quantityInput = '';
        }
        ...
        function submitEntry() {
                ...
                const planItem = $calculatedPlanItems.find((p: { article: string }) => p.article === selectedArticle);
                if (!planItem || planItem.locked) {
                        alert('План выполнен. Добавление невозможно.');
                        resetInput();
                        return;
                }
                
                const success = addWCEntry(welder.id, selectedArticle, quantity, today);
                if (!success) {
        ...
        function handleEntryClick(entry: WCEntry) {
                selectedArticle = entry.article;
                articleInput = entry.article;
                stage = 'quantity';
                quantityInput = '';
        }
        
        function handleLongPress(entry: WCEntry) {
                editModal = { show: true, entry };
        ...
        function openOvertimeModal() {
                if (!welder) return;
                overtimeModal = { show: true, date: currentOvertimeDate };
                overtimeInput = (welder.overtime[currentOvertimeDate] || 0).toString();
        }
        ...
        function handleResetOvertime() {
                if (!welder) return;
                resetOvertimeOverride(welder.id, overtimeModal.date);
                overtimeInput = (welder.overtime[overtimeModal.date] || 0).toString();
        }
        ...
        function isEntryLocked(article: string): boolean {
                return $calculatedPlanItems.find((p) => p.article === article)?.locked ?? false;
        }
...
<svelte:head>
        <title>WeldTrack - {welder?.lastName || 'Карточка'}</title>
</svelte:head>
...
<div class="page-container">
                        ...
                        <div class="input-area">
                                ...
                                {:else}
                                        <div class="info-panel">
                                                ...
                                                <div class="info-details">
                                                        ...
                                                        <span class="info-progress">
                                                                {#each selectedProgress as p, i}
                                                                        {#if i > 0}, {/if}{p.lastName}: {formatQty(p.quantity)}
                                                                {/each}
                                        ...
                                        <div class="quantity-input-row">
                                                ...
                                                <button class="btn btn-secondary" onclick={resetInput}>
                                                        Отмена
                                                </button>
                                        </div>
                                {/if}
        ...
        <div class="scrollable-content page-content">
                {#if !welder}
                        <div class="empty-state">
                                ...
                                <div class="empty-state-text">Сварщик не найден</div>
                        </div>
                        ...
                        <div class="entries-list">
                                {#each sortedEntries as entry (entry.id)}
                                        ...
                                        <div 
                                                ...
                                                use:longpressAction={() => handleLongPress(entry)}
                                        >
                                                <span class="entry-article" class:article-locked={isLocked}>{entry.article}</span>
                                                {#if isLocked}
                                                        <span class="locked-label">(план выполнен)</span>
                                                {/if}
        ...
        <div class="overtime-display" onclick={openOvertimeModal}>
                ...
                <div class="overtime-value" class:zero={currentOvertime === 0}>
                        {formatQty(currentOvertime)} ч
                </div>
        </div>
</div>
...
<div class="modal-overlay" onclick={closeEditModal}>
        <div class="modal-content" onclick={(e) => e.stopPropagation()}>
                ...
                <div class="modal-info">
                        <span class="article-badge">{editModal.entry.article}</span>
                        <span class="info-text">Дата: {editModal.entry.date}</span>
                </div>
...
<div class="modal-overlay" onclick={closeOvertimeModal}>
        <div class="modal-content" onclick={(e) => e.stopPropagation()}>
                ...
                <div class="modal-info">
                        <span class="info-text">Дата: {overtimeModal.date}</span>
                </div>
                <div class="modal-body">
                        <div class="form-group">
                                <label>Часы переработки</label>
                                <input 
                                        type="text"
                                        class="input-field"
                        ...
                        {#if welder?.manualOvertimeOverrides?.[overtimeModal.date]}
                                ...
                                <button class="btn btn-secondary w-full" onclick={handleResetOvertime}>
                                        Сбросить автоматический расчёт
                                </button>
                        {/if}
                </div>
                <div class="modal-actions">
...
<style>
        ...
        .quantity-input-row .input-field {
                flex: 1;
        }
        ...
        .overtime-display {
                ...
                background: linear-gradient(to right, transparent, var(--color-bg-secondary) 30%);
                padding: 0.75rem 1rem;
                padding-left: 2rem;
                z-index: 50;
                cursor: pointer;
